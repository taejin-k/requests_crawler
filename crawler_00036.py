...
def filter_01(row):
    title = row.select('.title > a')[0].text.replace("'", "").strip().replace('“','"').replace("‘", "").replace('”', '"').replace("’", "").replace('\xa0', '')

    if row.select('.mov') != []:
        chul_cheo = row.select('.mov')[0].text.split('\t\t\t\t\t\n')[0].strip()
    else:
        chul_cheo = row.select('.spon_media')[0].text.strip()

    url = 'https://news.zum.com' + row.select('.title > a')[0].get('href')

    date_time = row.find('span').text.split('\t\t\t\t\t\n')[1].strip().replace('.', '-') + ':00'
...
def crawling():
    ...
    try:
        ...
        for page in range(1, MAX_PAGE + 1):
            ...
            list_url = "https://news.zum.com/latest?p={}".format(page)
            response = requests.get(list_url, headers=headers)

            if response.status_code == 200:
                html = BeautifulSoup(response.text, 'html.parser')
                rows = html.select("#content > div.newest1 > div > ul > li")
                ...
                for row in rows:
                    ...
                    if last_dict.get(content_url):
                        ...
                        if response_content.status_code == 200:
                            ...
                            if content_html.select("#article_body") == []:
                                continue
                            ...
                            content_html = BeautifulSoup(response_content.text, 'html.parser')
                            if content_html.select('.news_del') != []:
                                logger.info('삭제된 기사입니다.')
                            else:
