...
def filter_01(row):
    ...
    content_url = 'http://news.jtbc.joins.com' + row.select('dl > dt > a')[0].attrs['href']

    return title, content_url


def filter_02(content_html):
    content = content_html.select("#articlebody > div:nth-of-type(1)")[0]
    div_tag_list = content.find_all('div')
    for div_tag in div_tag_list:
        div_tag.extract()
    content = content.text.strip()

    reg_datetime = content_html.select("#articletitle > div > span > span")[0].text.replace('입력 ', '') + ':00'

    chul_cheo = 'JTBC'

    return content, reg_datetime, chul_cheo
...
def crawling():
    ...
    try:
        ...
        for page in range(1, MAX_PAGE + 1):
            ...
            list_url = "http://news.jtbc.joins.com/section/list.aspx?scode=&pgi={}".format(page)
            ...
            if response.status_code == 200:
                ...
                rows = html.select("#section_list > li")
                ...
                for row in rows:
                    ...
                    if response_content.status_code == 200:
                        ...
                        content, reg_datetime, chul_cheo = filter_02(content_html)
