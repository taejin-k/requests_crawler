...
SU_JIB_WON_ID = 32
MAX_PAGE = int(os.getenv('CATCHONDATA_1_MAX_PAGE', '4'))
ROW_PER_PAGE = int(os.getenv('CATCHONDATA_1_ROW_PER_PAGE', '5'))
...
def filter_01(row):
    ...
    reg_datetime = row.select('span')[0].text.replace('.', '-') + ':00'

    return content_url, title, reg_datetime


def filter_02(content_html):
    jag_seong_ja = content_html.select('#container > div.view_top > div > div.info > div > div > ul > li')[0].text

    content = content_html.select('#textBody')[0]
    ...
    return content, jag_seong_ja, chul_cheo
...
def crawling():
    ...
    try:
        ...
        for page in range(1, MAX_PAGE + 1):
            ...
            list_url = "https://news.mt.co.kr/newsTotalList.html?pDepth1=newsTotal"
            response = requests.get(list_url, headers=headers, verify=False)

            if response.status_code == 200:
                html = BeautifulSoup(response.text, 'html.parser')
                rows = html.select("#content > ul > li:nth-of-type({}) > ul > li".format(page))

                for row in rows:

                    content_url, title, reg_datetime = filter_01(row)
                    ...
                    if last_dict.get(content_url):
                        ...
                        if response_content.status_code == 200:
                            ...
                            content, jag_seong_ja, chul_cheo = filter_02(content_html)
