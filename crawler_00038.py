...
def filter_01(row):
    title = row.select('td:nth-child(3) > a')[0].text.strip()

    chul_cheo = '공정거래위원회 ' + row.select('td:nth-child(4)')[0].text

    url = 'http://www.ftc.go.kr/www/selectReportUserView.do?key=10&rpttype=1&report_data_no=' + row.select('td:nth-child(3) > a')[0].get('href').split("'")[1]

    date_time = row.select('td:nth-child(5)')[0].text + " {:%H:%M:%S}".format(datetime.now())

    return title, chul_cheo, url, date_time


def filter_02(content_html):
    content = content_html.select("#formx > fieldset > table tr:nth-child(6) td")[0].text.strip()
...
def crawling():
    ...
    try:
        ...
        for page in range(1, MAX_PAGE + 1):
            ...
            list_url = "http://www.ftc.go.kr/www/ReportUserList.do?key=10&rpttype=1".format(page)
            ...
            if response.status_code == 200:
                ...
                rows = html.select("#formx > fieldset:nth-of-type(2) > table > tbody > tr")
